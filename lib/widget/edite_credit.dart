...
class EditeCredit extends StatefulWidget {
  final Utilisateur user;
  final CreditsModels creditsModels;
  final String idCredit;
  const EditeCredit(
      {super.key,
      required this.user,
      required this.creditsModels,
      required this.idCredit});

  @override
  State<EditeCredit> createState() => _EditeCreditState();
}

class _EditeCreditState extends State<EditeCredit> {
  var service = ServiceApi();
  bool showSpine = false;

  @override
  void initState() {
    numcompte = TextEditingController(text: widget.creditsModels.nomcompte);
    codeagence = TextEditingController(text: widget.creditsModels.codeagence);
    nomprenom = TextEditingController(text: widget.creditsModels.nomprenom);
    age = TextEditingController(text: widget.creditsModels.age);
    lieu = TextEditingController(text: widget.creditsModels.lieu);
    revenu = TextEditingController(text: widget.creditsModels.revenu);
    montantcollicite =
        TextEditingController(text: widget.creditsModels.loanAmount);
    taux = TextEditingController(text: widget.creditsModels.taux);
    duree = TextEditingController(text: widget.creditsModels.loanTerm);
    quotite = TextEditingController(text: widget.creditsModels.quotite);
    nombreenfcharge =
        TextEditingController(text: widget.creditsModels.npaCharge);
    creditcontract = TextEditingController(text: widget.creditsModels.loanNbr);

    selectedTypeCredit = widget.creditsModels.typecredit;
    selectedCatCredit = widget.creditsModels.catcredit;
    selectedLogement = widget.creditsModels.logement;
    selectedMatri = widget.creditsModels.situationmatri;
    selectedGenre = widget.creditsModels.sexe;
    selectedSecteurProf = widget.creditsModels.secteurprof;
    selectedNiveau = widget.creditsModels.niveauetude;
    haveSecondActi = widget.creditsModels.activiteSecondaire == "Oui" ? 0 : 1;
    isEmployeeSahel = widget.creditsModels.isEmployeeSahel == "Oui" ? 0 : 1;
    ;
    super.initState();
  }

  TextEditingController? numcompte;
  TextEditingController? codeagence;
  TextEditingController? nomprenom;
  TextEditingController? age;
  TextEditingController? lieu;
  TextEditingController? revenu;
  TextEditingController? montantcollicite;
  TextEditingController? taux;
  TextEditingController? duree;
  TextEditingController? quotite;
  TextEditingController? nombreenfcharge;
  TextEditingController? creditcontract;
  int? haveSecondActi;
  int? isEmployeeSahel;
                  ...
                  FilledButton(
                    ...
                    onPressed: () {
                      ...
                      if (numcompte!.text.isEmpty ||
                        ...
                        } else if (int.tryParse(nombreenfcharge!.text) ==
                            null) {
                          alertDialogue(context,
                              content: "Le champ duree doit être un entier");
